...
<table width="100%">
 	<tr>
 		<td>
 			<img src="../assets/img/images.jfif" width="150">
 		</td>
 		<td>
 				...
 				<h1>
 					ABSESNSI SISWA <br>
 					<small> SMP NEGERI 2 CAMPALAGIAN</small>
 				</h1>
 				<hr>
 				<em>
				 l.poros Majene Desa Baru, Kecamatan Luyo,<br> Kabupaten Kabupaten Polewali Mandar, Provinsi Sulawesi Barat, Kode Pos 91353 <br>
 				<b>Email :  smpn2campalagian@gmail.com</b> 
 				</em>
...
<i class="fas fa-print"></i>
 ...
 REKAP (<?=strtoupper($d['semester']);?> - <?=strtoupper($d['tahun_ajaran']);?>)
			...
			// }
			 ...
			 foreach ($qry as $bulan) { ?>
			 	<?php 
			 	$bulan = isset($rowBulan['tgl_absen']) ? date('m', strtotime($rowBulan['tgl_absen'])) : '01';
				$tglBulan = $rowBulan['tgl_absen'];
				$tglTerakhir = isset($tglBulan) && $tglBulan ? date('t', strtotime($tglBulan)) : 30;
...
CETAK BULAN (<?php echo strtoupper(namaBulan($bulan));?>)
				...
				<table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
				  <tr>
				    ...
				    <td colspan="<?=$tglTerakhir;?>" style="padding: 8px;">PERTEMUAN KE- DAN BULAN : 
<b style="text-transform: uppercase;">
    <?php echo namaBulan($bulan);?> 
    <?= isset($tglBulan) && $tglBulan ? date('Y', strtotime($tglBulan)) : date('Y'); ?>
</b>
</td>
